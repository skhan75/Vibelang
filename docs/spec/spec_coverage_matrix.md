...
| SPEC-CNT-001 | Containers | Dynamic `List`/`Map` operations have deterministic semantics | implemented | `docs/spec/containers.md`, `reports/v1/dynamic_containers_conformance.md`, workflow `v1-release-gates.yml` job `dynamic_containers_gate` | Implemented for the `7.3.f.1` freeze scope (`List<Int>`, `Map<Int, Int>`, `Map<Str, Int>`, and deterministic ordering policy) |
