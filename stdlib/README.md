...
- `json` (preview): `Json` parse/stringify (+ pretty), `json.builder` for dynamic JSON, typed codecs, `try_parse` for network bytes, utilities (`is_valid`, `parse_i64`/`stringify_i64`, `minify`)
